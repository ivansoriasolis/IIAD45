...
class SortedTableMap(MapBase):
  """Implementación de un mapa usando una tabla ordenada."""

  #----------------------------- comportamientos no públicos -----------------------------
  def _find_index(self, k, low, high):
    """Devuelve el índice del elemento más a la izquierda con clave mayor o igual que k.

    Devuelve high + 1 si ningún elemento califica.

    Es decir, se devolverá j tal que:
       todos los elementos de table[low:j] tienen clave < k
       todos los elementos de table[j:high+1] tienen clave >= k
    """
    if high < low:
      return high + 1                               # ningún elemento califica
    else:
      mid = (low + high) // 2 
  ...
  def __setitem__(self, k, v):
    ...
    if j < len(self._table) and self._table[j]._key == k:
      self._table[j]._value = v                         # reasignar valor
    else:
      self._table.insert(j, self._Item(k,v))            # agrega nuevo elemento
  ...
  def __delitem__(self, k):
    ...
    if j == len(self._table) or self._table[j]._key != k:
      raise KeyError('Key Error: ' + repr(k))
    self._table.pop(j)                                  # eliminar elemento
  ...
  def find_range(self, start, stop):
    ...
    if start is None:
      ...
      j = self._find_index(start, 0, len(self._table)-1)   # encontrar el primer resultado
