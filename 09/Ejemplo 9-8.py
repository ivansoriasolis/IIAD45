# Ejemplo 9.8 Ejemplo de implementacion de cola de prioridad adaptable
...
class AdaptableHeapPriorityQueue(HeapPriorityQueue):
  ...
  def remove(self, loc):
    ...
    if j == len(self) - 1:                # item a la última posición
      self._data.pop()                    # removerlo
    else:
      self._swap(j, len(self)-1)          # intercambiar con el último
      self._data.pop()                    # removerlo de la lista
      self._bubble(j)                     # reparar la propiedad del heap
